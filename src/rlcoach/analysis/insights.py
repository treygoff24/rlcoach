...
def generate_team_insights(
    per_team_analysis: dict[str, Any],
    per_player_analysis: dict[str, Any]
) -> list[dict[str, Any]]:
...
def _analyze_positioning_insights(positioning: dict, player_id: str) -> list[dict[str, Any]]:
    ...
    if first_man_pct > 60:
        insights.append({
            "severity": "WARNING",
            "message": "High first-man presence may indicate ball-chasing tendencies - consider rotating back more frequently.",
            "evidence": {
                "first_man_pct": first_man_pct
            }
        })
...
def _analyze_boost_insights(boost: dict, player_id: str) -> list[dict[str, Any]]:
    ...
    if time_zero_boost_s > 60:
        insights.append({
            "severity": "SUGGESTION",
            "message": f"Reduce time at zero boost ({time_zero_boost_s:.1f}s) by improving boost collection patterns and efficiency.",
            "evidence": {
    ...
    if waste > 200:
        insights.append({
            "severity": "INFO",
            "message": f"Consider optimizing boost usage - detected {waste:.0f} units of inefficient boost consumption.",
            "evidence": {
                "boost_waste": waste
            }
        })
    ...
    if stolen_big_pads >= 3:
        insights.append({
            "severity": "INFO",
            "message": f"Good boost denial - stole {stolen_big_pads} corner boosts from opponents.",
            "evidence": {
...
def _analyze_movement_insights(movement: dict, player_id: str) -> list[dict[str, Any]]:
    ...
    powerslide_count = movement.get("powerslide_count", 0)
    ...
    if total_time > 0:
        ...
        if slow_pct > 60:
            insights.append({
                "severity": "SUGGESTION",
                "message": f"Increase momentum and speed - {slow_pct:.1f}% of time spent at low speed.",
                "evidence": {
    ...
    if aerial_count < 5 and total_time > 300:  # 5+ minute match
        insights.append({
            "severity": "INFO",
            "message": f"Limited aerial attempts ({aerial_count}) - consider more aerial challenges and shots.",
            "evidence": {
...
def _analyze_rotation_insights(rotation_compliance: dict, player_id: str) -> list[dict[str, Any]]:
    ...
    if score < 70:
        insights.append({
            "severity": "WARNING",
            "message": f"Rotation compliance score is {score:.1f}/100 - focus on positioning and team coordination.",
            "evidence": {
    ...
    if "double_commit" in str(flags):
        insights.append({
            "severity": "SUGGESTION",
            "message": "Multiple double-commit violations detected - improve communication and positioning awareness.",
            "evidence": {
                "violation_type": "double_commit",
                "flags": flags
            }
        })
...
def _analyze_fundamentals_insights(fundamentals: dict, player_id: str) -> list[dict[str, Any]]:
    ...
    if shooting_percentage < 20 and shots >= 5:
        insights.append({
            "severity": "SUGGESTION", 
            "message": f"Shooting accuracy is {shooting_percentage:.1f}% - focus on shot placement and timing.",
            "evidence": {
    ...
    if shooting_percentage > 40 and shots >= 3:
        insights.append({
            "severity": "INFO",
            "message": f"Excellent shooting accuracy at {shooting_percentage:.1f}% - maintain quality shot selection.",
            "evidence": {
                "shooting_percentage": shooting_percentage,
                "shots": shots
            }
...
def _analyze_challenges_insights(challenges: dict, player_id: str) -> list[dict[str, Any]]:
    ...
    if first_to_ball_pct < 40 and contests >= 10:
        insights.append({
            "severity": "SUGGESTION",
            "message": f"First-to-ball rate is {first_to_ball_pct:.1f}% - work on reading plays and positioning for challenges.",
            "evidence": {
    ...
    if contests > 0:
        ...
        if win_rate > 60 and contests >= 8:
            insights.append({
                "severity": "INFO",
                "message": f"Strong challenge success rate at {win_rate:.1f}% - good mechanical execution.",
                "evidence": {
...
def _analyze_team_fundamentals(blue_data: dict, orange_data: dict) -> list[dict[str, Any]]:
    ...
    if blue_shooting - orange_shooting > 15:
        insights.append({
            "severity": "INFO",
            "message": f"BLUE team shooting advantage: {blue_shooting:.1f}% vs {orange_shooting:.1f}%",
            "evidence": {
        ...
        insights.append({
            "severity": "INFO", 
            "message": f"ORANGE team shooting advantage: {orange_shooting:.1f}% vs {blue_shooting:.1f}%",
            "evidence": {
                "blue_shooting": blue_shooting,
                "orange_shooting": orange_shooting
            }
...
def _analyze_team_boost(blue_data: dict, orange_data: dict) -> list[dict[str, Any]]:
    ...
    if blue_stolen - orange_stolen > 500:
        insights.append({
            "severity": "INFO",
            "message": f"BLUE team boost control advantage: {blue_stolen:.0f} vs {orange_stolen:.0f} boost stolen",
            "evidence": {
        ...
        insights.append({
            "severity": "INFO",
            "message": f"ORANGE team boost control advantage: {orange_stolen:.0f} vs {blue_stolen:.0f} boost stolen",
            "evidence": {
                "blue_stolen": blue_stolen,
                "orange_stolen": orange_stolen
            }
...
def _analyze_team_kickoffs(blue_data: dict, orange_data: dict) -> list[dict[str, Any]]:
    ...
    if total_kickoffs >= 3:
        if blue_first >= orange_first + 2:
            insights.append({
                "severity": "INFO",
                "message": f"BLUE kickoff advantage: won first possession {blue_first}/{total_kickoffs} times",
                "evidence": {
            ...
            insights.append({
                "severity": "INFO",
                "message": f"ORANGE kickoff advantage: won first possession {orange_first}/{total_kickoffs} times", 
                "evidence": {
                    "blue_first_possession": blue_first,
                    "orange_first_possession": orange_first
                }
...
def _analyze_team_positioning(blue_data: dict, orange_data: dict) -> list[dict[str, Any]]:
    ...
    if blue_ahead > 50 and orange_ahead > 50:
        insights.append({
            "severity": "WARNING",
            "message": "Both teams showing high ahead-of-ball positioning - consider more defensive structure",
            "evidence": {
