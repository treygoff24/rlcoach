...
async def list_players(
    ...
    """List players (excluding self) encountered in the user's replays.

    Requires authentication. Only returns players seen in this user's replays.
    ...
    # Select player IDs appearing in this user's replays
    player_ids_in_replays = select(PlayerGameStats.player_id).where(
        PlayerGameStats.replay_id.in_(user_replay_ids)
    )

    query = db.query(Player).filter(
        Player.is_me.is_(False),
        Player.player_id.in_(player_ids_in_replays),
    )
...
async def get_player(
    ...
    Requires authentication. Player must appear in the user's replays.
    ...
    # Verify player appears in user's replays
    player_in_replays = (
    ...
    # Compute tendency profile using only stats from this user's replays
    stats = (
        db.query(PlayerGameStats)
        .filter(
            PlayerGameStats.player_id == player_id,
